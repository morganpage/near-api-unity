...
namespace NearClientUnity.KeyStores
{
  public class UnencryptedFileSystemKeyStore : KeyStore
  {
    ...
    public static async Task<JObject> LoadJsonFile(string path)
    {
    ...
    public static async Task<JArray> ReadKeyFile(string path)
    {
      var accountInfo = await LoadJsonFile(path);
      string privateKey = accountInfo["PrivateKey"]?.ToString();
      if (privateKey == null && accountInfo["SecretKey"] != null) privateKey = accountInfo["SecretKey"].ToString();
      return new JArray { accountInfo["AccountId"], KeyPair.FromString(privateKey) };
    }
