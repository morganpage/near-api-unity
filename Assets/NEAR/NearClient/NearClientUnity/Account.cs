...
namespace NearClientUnity
{
  public class Account
  {
    ...
    public async Task FetchStateAsync()
    {
      ...
      if (publicKey == null)
      {
        Debug.Log("FetchStateAsync: PublicKey is null");
        return;
      }
      try
      {
      ...
      {
        Debug.Log("FetchStateAsync: " + exception.Message);
        throw new Exception(
            $"Failed to fetch access key for '{_accountId}' with public key {publicKey.ToString()}");
      }
    }

    public async Task<FinalExecutionOutcome> FunctionCallAsync(string contractId, string methodName, JObject args, ulong? gas = null, Nullable<UInt128> amount = null)
    {
      if (args == null)
      {
        args = new JObject();
    ...
    public async Task<JObject> GetAccessKeysAsync()
    {
    ...
    {
      var methodArgs = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(args));
      JObject response = await _connection.Provider.QueryAsync($"call/{contractId}/{methodName}", Base58.Encode(methodArgs));
      var result = response;
      return result;
    }
